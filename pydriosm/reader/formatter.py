...
def reformat_multipolygon_point(coords):
    ...
    **Examples**::
        ...
        >>> coords_ = reformat_multipolygon_point(coords)
        >>> coords_
    ...
    **Examples**::
    ...
    if layer_name == 'other_relations':
        if isinstance(layer_data, pd.DataFrame):  # geom_col_name in layer_data.columns:
            ...
            geom_data = geometries.map(convert_geometry_collection)
            geom_data.name = geom_col_name
        else:
            geom_data = layer_data.map(lambda x: convert_geometry_collection(x, mode=2))

    else:  # `layer_data` can be 'points', 'lines', 'multilinestrings' or 'multipolygons'
        if isinstance(layer_data, pd.DataFrame):  # geom_col_name in layer_data.columns:
            geom_data = layer_data[geom_col_name].map(convert_simplex_geometry)
        else:
            geom_data = layer_data.map(lambda x: convert_simplex_geometry(x, mode=2))
...
def refresh_other_tags(prop_or_feat, mode=1):
    ...
    **Examples**::
    ...
    if mode == 1:
        ...
        properties.update({'other_tags': reformat_other_tags(properties['other_tags'])})
        ...
        feat_ = update_dict(feat, {'properties': {'other_tags': reformat_other_tags(other_tags)}})
